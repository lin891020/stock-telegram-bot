...
| **產出後稽核** | 模型寫出證據包裡沒有的數字 | 抓出既不在證據包、也推導不出來的數字 |
...
`bot/services/consistency.py` 的五項規則：營業現金流不得大於營收、
營收 ≥ 毛利 ≥ 營業利益 ≥ 淨利、資產 = 負債 + 權益、資本支出應為負數、
兩個來源差三倍以上即量級錯誤。原則跟證據包一致：**只標記，不猜哪個對**。

`bot/services/claim_audit.py` 的難點不在抓編造，而在**不要把合法推導誤報成
編造**——報告裡的比率、成長率本來就是算出來的。它會實際把推導算一遍再判斷，
實測把 17 個誤報壓到 1 個（而那一個標記是正確的）。同樣只標記、不改寫報告：
改寫要用模型驗模型，會引入新的錯誤。
