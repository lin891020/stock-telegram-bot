...
四則定時推播，時間都可用 `/settime` 改（台北時間，週末不推）：

| 時間 | 推什麼 | 為什麼是這個時間 |
|---|---|---|
| **05:30** | 美股收盤速報 | 美股 16:00 ET 收盤 ＝ 台北 04:00（夏令）／05:00（冬令），05:30 兩種都涵蓋 |
| **06:30** | 起床報 | 今天要看＋大盤＋自選股行情＋異動股的新聞 |
| **12:00** | 台股盤中速報 | 台股 09:00–13:30 開盤，中午看一次進度 |
| **14:00** | 台股收盤速報 | 13:30 收盤，留緩衝給結算 |

起床報最上面一行是「⚠️ 今天要看」，把當天異動 ±3% 的股票濃縮成一行。
新聞只給異動的那幾支——以前的規則是「有新聞或有異動」，但每支股票每天
都有新聞，於是十支列了九支，收合機制形同失效（實測 67 行、76% 是新聞）。

> ⚠️ 盤中速報與價格提醒**不能**走 `get_stock_summary`。台股報價來自 TWSE
> 盤後結算，中午查只會拿到前一個交易日的收盤，而畫面上看不出來。
> 那條路要用 `intraday_quote`。
...
| **每日推播的內容** | `handlers/digest.py` |
| **每日推播的時間** | `handlers/schedule.py` 的 `_JOBS`，加一筆就多一個排程 |
| **盤中即時報價** | `services/stock.py` 的 `intraday_quote`（⚠️ 別改回 `fast_info.previous_close`，見該處註解） |
